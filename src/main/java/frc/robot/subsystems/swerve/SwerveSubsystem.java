...
public class SwerveSubsystem extends SubsystemBase {
  ...
  public SwerveSubsystem(File directory) {
    ...
    try {
      swerveDrive = new SwerveParser(directory).createSwerveDrive(4.5,
  ...
  public void periodic() {
    ...
    try {
      ...
      double headingErrorRadians = swerveDrive.swerveController.config.headingPIDF.p > 0
          ? (currentHeading - swerveDrive.getOdometryHeading().getRadians())
          : 0;
  ...
  public void setupPathPlanner() {
    ...
    try {
      ...
      autoPathFollower = // Method that will drive the robot given ROBOT RELATIVE ChassisSpeeds. Also
    ...
    rotationPIDHelper = new PIDHelper(
        ...
        (p) -> {
          ...
          try {
            var controller = swerveDrive.swerveController.getClass().getDeclaredField("headingController");
            controller.setAccessible(true);
            var pidController = (edu.wpi.first.math.controller.PIDController) controller
   ...
   * 
   * @param targetAngleDegrees Target heading in degrees (0 = forward, 90 = left,
   *                           etc.)
   ...
   * Driver can move in any direction, and the robot will maintain the target
   * heading.
   * This is commonly called "heading lock" or "cardinal lock".
   * 
   * @param targetAngleDegrees   Target heading in degrees (0 = forward, 90 =
   *                             left, etc.)
  ...
  /**
   * Resets the gyro angle to zero and resets odometry to the same posi\[]\tion,
   * but facing toward 0.
   */
  public void zeroGyro() {
    System.out.println("BLUD");

    // swerveDrive.setGyro(new
    // Rotation3d().minus(swerveDrive.getGyro().getRawRotation3d()));
